...
| App 版本 | v0.2.1 |
| 安装包 | `invoice-reimbursement-0.2.1-macos-arm64.dmg` |
| 邮箱 | Gmail 或 QQ 邮箱 |
...
v0.2.1 是用于本地分享的 Apple Silicon macOS 安装包。App 采用 ad-hoc（临时）签名并启用了 Hardened Runtime，但未使用 Apple Developer ID 签名，也未经过 Apple 公证。macOS 第一次打开时可能会阻止运行。

- 只从你信任的分享者处获取安装包。
- 不要为了运行本 App 关闭 Gatekeeper 或 macOS 的整体安全保护。
- 分享者应同时提供校验文件 `invoice-reimbursement-0.2.1-macos-arm64.dmg.sha256`。

将 DMG 和校验文件放在同一目录。熟悉终端的用户可以进入该目录并执行：

~~~bash
shasum -a 256 -c invoice-reimbursement-0.2.1-macos-arm64.dmg.sha256
~~~

校验成功时，输出应包含 `invoice-reimbursement-0.2.1-macos-arm64.dmg: OK`。校验失败时不要安装，应重新从可信分享者处获取 DMG 和校验文件。
...
本手册对应《发票报销》v0.2.1，更新日期为 2026-07-23。如果界面按钮、支持邮箱或导出结构在新版本中发生变化，请以新版本随附手册为准。
