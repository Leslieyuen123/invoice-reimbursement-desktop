...
1. 从分享者处获取 `invoice-reimbursement-0.2.5-macos-arm64.dmg`，打开后将《发票报销》拖入“应用程序”。
...
**安全与校验：** 当前分享版采用 ad-hoc（临时）签名并启用了 Hardened Runtime，未使用 Apple Developer ID 签名，也未经过 Apple 公证。只从可信分享者处获取安装包，不要关闭 Gatekeeper。分享者应同时提供 `invoice-reimbursement-0.2.5-macos-arm64.dmg.sha256`；将它和 DMG 放在同一目录，在终端运行 `shasum -a 256 -c invoice-reimbursement-0.2.5-macos-arm64.dmg.sha256`，结果应显示 `OK`。

更多说明请查看《发票报销完整用户手册》。

本快速入门对应《发票报销》v0.2.5，更新日期为 2026-09-16。请以当前版本随附文档为准。
